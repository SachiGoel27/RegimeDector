...
**Research Question**
Can we detect latent market regimes by clustering the time-varying correlation structure of equity sectors? Do transitions into a "high-correlation" regime predict subsequent volatility spikes or market drawdowns?
...
Instead of feeding the HMM raw prices, we engineered a robust set of 19 features designed to capture the character of the market's correlation structure. These were calculated on a 60-day rolling window:
...
To solve this, we applied Principal Component Analysis (PCA) to transform the 19 correlated features into a smaller set of uncorrelated "Principal Components."
...
- Why an HMM? It assumes the market is always in one of several "states" (regimes) and that the features we see (our 6 PCs) are generated by that state. The model's job is to learn the properties of these states and predict which state we are in at any given time.
...
This is the "moment of truth." We check if the regime today has any predictive power over the market's outcome 21 trading days (1 month) into the future.
...
- Conclusion: Since p < 0.05, we reject the null hypothesis. The relationship between the regime and the probability of a drawdown is statistically significant.

**Finding 4: Benchmark Comparison (HMM vs. Simple Models)**
The HMM's findings (High-Corr = High Vol, High Return, Low Risk) are robust. But is the HMM necessary? We compared its results to two simpler "benchmark" models:
